;(function () {
  ...
  OTR.prototype._smInit = function () {
    ...
    this.sm.on('send', function (ssid, send) {
      if (self.ssid === ssid) {
        self.prepareMsg(send).then(function(msg){
          self.io(msg)
        })
      }
    })
    ...
    return new Promise(
      function(resolve, reject) {
        ...
        if (this.msgstate !== CONST.MSGSTATE_ENCRYPTED || msg.length !== 8) {
          ...
          reject('Received an unreadable encrypted message.')
          return
        }
        ...
        if (our_keyid < 0 || our_keyid > 1) {
          ...
          reject('Not of your latest keys.')
          return
        }
        ...
        if (ctr <= sessKeys.rcv_counter) {
          ...
          reject('Counter in message is not larger.')
          return
        }
        ...
        HLP.decryptAesAsync(
          ...
          resolve(out.toString(CryptoJS.enc.Utf8))
        }.bind(this))
  ...
  OTR.prototype.sendMsg = function (msg, meta) {
    ...
    switch (this.msgstate) {
      ...
      case CONST.MSGSTATE_ENCRYPTED:
        this.prepareMsg(msg).then(function(msg){
          this.io(msg, meta)
        }.bind(this))
        return
      default:
  ...
  OTR.prototype.sendStored = function () {
    ...
    ;(this.storedMgs.splice(0)).forEach(function (elem) {
      self.prepareMsg(elem.msg).then(function(msg){
  ...
  OTR.prototype.sendFile = function (filename) {
    ...
    this.prepareMsg(msg, filename).then(function (msg) {
      this.io(msg)
    }.bind(this))
  }
