;(function () {
  ...
  HLP.convertWordArrayToArrayBuffer = function (wordArray) {
    var buffer = new ArrayBuffer(wordArray.sigBytes);
    var targetArray = new Uint8Array(buffer);

    for (var i = 0; i < wordArray.sigBytes; ++i) {  // copy byte by byte from left as the last word could be incomplete
      targetArray[i] = (wordArray.words[Math.floor(i / 4)] >> (3 - (i % 4)) * 8) & 0xff
    }

    return buffer;
  }

  HLP.convertArrayBufferToWordArray = function (arrayBuffer) {
    var sourceArray = new Uint8Array(arrayBuffer);
    var targetArray = new CryptoJS.lib.WordArray.init(undefined, arrayBuffer.byteLength);

    for (var i = 0; i < arrayBuffer.byteLength; ++i) {
      if (i % 4 == 0) {
        targetArray.words[Math.floor(i / 4)] = 0;  // init element
      }
      targetArray.words[Math.floor(i / 4)] += sourceArray[i] << ((3 - (i % 4)) * 8);
    }

    return targetArray;
  }

  HLP.importAesKey = function(c) {  // type of key is string
      var keyBuffer = HLP.convertWordArrayToArrayBuffer(CryptoJS.enc.Latin1.parse(c));
  ...
  HLP.encryptAesAsync = function (msg, c, iv) {  // msg is wordarray, c and iv are string
    // example: https://github.com/diafygi/webcrypto-examples#aes-ctr---importkey
    return new Promise(
      function(resolve, reject) {
        HLP.importAesKey(c).then(function (key) {
          console.log("AsyncCrypto: imported key: " + key);
          var msgBuffer = HLP.convertWordArrayToArrayBuffer(msg);
          var ivBuffer = HLP.convertWordArrayToArrayBuffer(CryptoJS.enc.Latin1.parse(iv));

          // start encryption async and notify encapsulating promise -- https://developer.mozilla.org/en-US/docs/Web/API/SubtleCrypto/encrypt
          crypto.subtle.encrypt(
            ...
            var wordArrayBuffer = HLP.convertArrayBufferToWordArray(encrypted)
            var result = CryptoJS.enc.Latin1.stringify(wordArrayBuffer)
            //console.log("AsyncCrypto: successfully encrypted: " + wordArrayBuffer + " / " + result.length);
            resolve(result);
          }).catch(function(err){
            console.error("AsyncCrypto: error encrypting: " + err);
            reject(err);
          });
        })
        .catch(function (err) {
          console.error("AsyncCrypto: error importing key: " + err);
          reject(err);
        });
      }
    );
  ...
  HLP.decryptAesAsync = function (msg, c, iv) {  // msg, c and iv are of type string
    // example: https://github.com/diafygi/webcrypto-examples#aes-ctr---importkey
    return new Promise(
        function(resolve, reject) {
          HLP.importAesKey(c).then(function (key) {
            console.log("AsyncCrypto: imported key: " + key);
            var msgBuffer = HLP.convertWordArrayToArrayBuffer(CryptoJS.enc.Latin1.parse(msg));
            var ivBuffer = HLP.convertWordArrayToArrayBuffer(CryptoJS.enc.Latin1.parse(iv));
            ...
            crypto.subtle.decrypt(
              ...
              var wordArrayBuffer = HLP.convertArrayBufferToWordArray(decrypted)
              var result = CryptoJS.enc.Latin1.stringify(wordArrayBuffer)
              //console.log("AsyncCrypto: successfully decrypted: " + wordArrayBuffer + " / " + result.length);
              resolve(result);
            }).catch(function(err){
              console.error("AsyncCrypto: error decrypting: " + err);
              reject(err);
            });
          })
          .catch(function (err) {
